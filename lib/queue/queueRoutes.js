...
router.post("/", function(req, res, next) {
    ...
    if (tokens > 0) {
        controller.addMusicToQueue(
            musicId, sessionID, tokens, function(err, tokens, message){
        ...
        res.send({
           message: "You need tokens to add songs to the queue. Wait a minute and try again"
        });
    }
});

router.post("/remove", function(req, res, next) {
    var queueId = req.body.queue;
    var sessionID = req.sessionID;
    controller.removeEntryFromQueue(queueId, sessionID, function(err, message) {
        res.send({
            message: message,
        });
    });
...
router.post("/up", function(req, res) {
    ...
    var tokens = req.session.tokens;
    if (tokens > 0) {
        controller.moveEntryUpQueue(queueId, function(err, message) {
            tokens--;
            req.session.tokens = tokens;
            res.send({
                message: message,
...
router.post("/down", function(req, res) {
    ...
    var tokens = req.session.tokens;
    if (tokens > 0) {
        controller.moveEntryDownQueue(queueId, function(err, message) {
            tokens--;
            req.session.tokens = tokens;
            res.send({
                message: message,
