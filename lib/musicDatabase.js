...
db.updatePlaylist = function(playlistName, musicPath) {
	db.Playlist.findOneAndUpdate({"name": playlistName}, {path:musicPath}, {upsert: true}, function (err, result){
		if (err) {
			console.log("MongoDB music error");
			db._handleError(err);
		}
		console.log("Updated: ", playlistName, " with: ", musicPath);
	});
};
