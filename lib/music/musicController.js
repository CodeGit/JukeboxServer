...
controller.getDistinctMusic = function(field, filter, page, callback) {
	console.log("getDistinctMusic(" + field + ", " + filter + ", " + page + ")");
	if (page == null) {
		page = 0;
	}
	var resultsPerPage = config.settings.search.resultsPerPage;
	var skipAhead = (page * resultsPerPage);
	var query = db.Music.distinct(field, {artist:1});
	var promise = query.exec();
	promise.addBack(function(err, hits){
		//console.log("Searched db for " + musicPath);
		if(err) { callback(err, hits); } 
		if (hits === null || hits === undefined) {
			...
			console.log("Hits = " + hits.length);
			if (callback != null) {
				var pageHits = hits.slice(skipAhead, (skipAhead+resultsPerPage));
				pageHits.unshift(hits.length);
				callback(null, pageHits);
			}
		}
	});
};
