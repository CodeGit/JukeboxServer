...
controller.getMusic = function(filter, page, callback) {
    ...
    if (filter != null && filter.hasOwnProperty('playlist')) {
        controller.getPlaylistMusic(filter, page, callback);
    }
    if (page == null) {
        page = 0;
    }
...
controller.getPlaylistMusic = function(filter, page, callback) {
    console.log("getPlaylistMusic(" + filter + ", " + page + ")");
    if (page == null) {
        page = 0;
    }
    var resultsPerPage = config.settings.search.resultsPerPage;
    var skipAhead = (page * resultsPerPage)
    var query = db.Playlist.find(filter)
        .skip(skipAhead)
        .limit(resultsPerPage)
        .populate("music")
        .sort({
...
controller.getPlaylistMusicCount =  function(filter, callback) {
    var query = db.PlaylistSummary.find(filter).select("music");
    controller.getCount(query, callback);
};
