...
router.get("/filter", function(req, res, next){
    ...
    var countMethod = controller.getMusicCount;
    var fetchMethod = controller.getMusic;
    var filter = {};
    ...
    if (req.query.playlist != null) {
        ...
        var countMethod = controller.getPlaylistMusicCount;
        var fetchMethod = controller.getPlaylistMusic;
    ...
    _fetchData({
        ...
        filter: filter,
        page: page,
        encodeFields: ["title", "album", "artist"],
        res: res,
        req: req,
        next: next,
        title: "Music"
...
router.get("/artist/filter", function(req, res, next){
    var page = req.query.page;
    if (page == null) {
        page = 0;
    }
    ...
    if (req.query.search != null) {
        filter.sortArtist = new RegExp(req.query.search, "i");
    }
    _fetchData({
        jade: "artists",
        countMethod: controller.getArtistCount,
...
router.get("/album/filter", function(req, res, next){
    var page = req.query.page;
    if (page == null) {
        page = 0;
    }
    var filter = {};
    if (req.query.search != null) {
        filter.album = new RegExp(req.query.search, "i");
    }
    _fetchData({
        jade: "albums",
        countMethod: controller.getAlbumCount,
...
var _fetchData = function (parameters) {
    ...
    var baseUrlPath = url.parse(req.originalUrl).pathname;
    var fullUrlPath = req.originalUrl;
    async.waterfall([
        function (callback) {
            //count hits
