...
db.initialise = function() {
    ...
    db.playListSchema.plugin(random, {path: 'r'});
    ...
    db.Session = mongoose.model("Session", db.SessionSchema);
};
...
db.updateMusic = function(data, createNew, callback) {
    ...
    music.sortTitle = data.sortTitle;
    music.artist = data.artist;
    if (music.artist) {
        music.sortArtist = music.artist.replace(/^The\s+/i, "");
        music.sortArtist = music.sortArtist.replace(/^\s+/, "");//strip leading spaces
    } else {
        music.sortArtist = music.artist;
    }
...
db.createQueueEntry = function(count, musicId, cookie, callback) {
    var queueEntry = new db.Queue();
    queueEntry.ordinal = count;
    queueEntry.music = musicId;
    queueEntry.session = cookie;
    queueEntry.save(function(err){
        callback(err, "Selected song is at number " + (count+1) + " in the queue");
    });
};

db.removeEntryFromQueue = function(queueId, session, callback) {
    console.log("Removing " + queueId);
    var query = db.Queue.findOneAndRemove({
        _id: queueId,
        session: session
    }).populate("music").lean();
    query.exec(function(err, hit){
        db.reOrderQueue(function(err){
            if (hit == null) {
                callback(err, "Entry not found in queue");
            } else {
                callback(err, "Removed " + hit.music.title + ": " + hit.music.artist);
            }
        })
    });
};

db.moveEntryUpQueue = function(queueId, callback) {
    console.log("Pushing " + queueId + " up queue");
    async.waterfall([
        function(waterfallCallback) {
            var query = db.Queue.find().sort({ordinal:1});
            db.findMatchAndPreviousEntries(query, queueId, waterfallCallback);
        }, function(current, previous, waterfallCallback) {
            db.swapOrdinals(current, previous, waterfallCallback);
        }, function(current, previous, waterfallCallback) {
            current.save(function(err) {
                previous.save(function(err){
                    waterfallCallback(err, "Successfully moved queue entry");
                });
            });
        }
...
db.moveEntryDownQueue = function(queueId, callback) {
    console.log("Pushing " + queueId + " down queue");
    async.waterfall([
        function(waterfallCallback) {
            var query = db.Queue.find().sort({ordinal:-1});
            db.findMatchAndPreviousEntries(query, queueId, waterfallCallback);
...
db.findMatchAndPreviousEntries = function(query, queueId, callback) {
    query.exec(function(err, hits) {
        var previous = null;
        for(var i = 0; i < hits.length; i++) {
            var current = hits[i];
            if (current._id.equals(queueId) && previous != null) {
                callback(null, current, previous);
                return;
            }
            previous = current;
        }
        callback("Failed to find queue entry", null, null);
    });
};

db.swapOrdinals = function(current, previous, callback) {
    var tempOrdinal = current.ordinal;
    current.ordinal = previous.ordinal;
    previous.ordinal = tempOrdinal;
    callback(null, current, previous);
};

db.reOrderQueue = function(callback) {
    var queue = db.Queue.find().sort({"ordinal": 1});
    var promise = queue.exec();
    var count = 0;
    promise.onResolve(function(err, hits){
        async.eachLimit(hits, 1, function(hit, iteratorCallback){
            hit.ordinal = count;
            count++;
...
db.getRandomSongFromPlaylist = function(playlist, callback) {
    console.log("Fetching random song from " + playlist);
    var query = db.Playlist.findRandom({playlist: playlist}).limit(1).populate("music").lean();
    var promise = query.exec();
    promise.onResolve(function(err, hits){
        console.log("Selected random " + hits[0].music.title);
        callback(err, hits[0].music);
    });
};
