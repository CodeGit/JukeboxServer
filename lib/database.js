...
db.createQueueEntry = function(count, musicId, cookie, callback) {
    ...
    queueEntry.user = cookie;
    queueEntry.save(function(err){
        callback(err, "Selected song is at number " + (count+1) + " in the queue");
    });
};

db.removeQueueEntry = function(musicId, cookie, callback) {

};

db.pushItemUpQueue = function(musicId, cookie, callback) {
    console.log("Pushing " + ordinal + " up queue");
}

db.pushItemDownQueue = function(musicId, cookie, callback) {
    console.log("Pushing " + ordinal + " cookie queue");
}
