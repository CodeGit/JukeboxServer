...
db.createMusicSummaries = function(err, callback) {
    ...
    promise.addBack(function(err, hits){
        async.eachLimit(hits, 1, function(hit, eachCallback){
            ...
            console.log("Finished creating createMusicSummaries");
            callback(err);
        });
    });
}
...
db.createPlaylistSummaries = function(err, callback) {
    var query = db.Playlist.find({}).select("name").lean();
    var promise = query.exec();
    promise.addBack(function(err, hits){
        async.eachLimit(hits, 1, function(hit, eachCallback){
...
db.findAndUpdatePlaylist = function(data, callback) {
    ...
    async.waterfall([
        function(waterfallCallback){
            db.PlaylistSummary.findOneAndUpdate({"name":data.name}, {"name": data.name}, {"upsert": true, "new": true}, function(err, hit){
                if (err) {waterfallCallback(err, hit);}
                waterfallCallback(null, hit);
            });
        },
        function(dbPlaylistData, waterfallCallback){
            if (dbPlaylistData.music === null || dbArtist.dbPlaylistData === undefined){
                dbPlaylistData.music = 0;
            }
            dbPlaylistData.music = dbPlaylistData.music + 1;

            dbPlaylistData.save(function(err){
                waterfallCallback(err);
            });
        }
    ], function(err){
        if (err){callback(err);}
        console.log("Finished Playlist " + data.name + " update");
        callback();
    });
};
