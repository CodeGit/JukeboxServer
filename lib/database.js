...
db.createQueueEntry = function(count, musicId, user, title, callback) {
    ...
    queueEntry.save(function(err){
        callback(err, title + " is at number " + (count+1) + " in the queue");
    });
};
...
db.pushItemDownQueue = function(musicId, callback) {
    console.log("Pushing " + ordinal + " down queue");
}
