...
db.updateSummaries = function(callback) {
	...
	async.waterfall([
		function(waterfallCallback) {
            ...
            db.createSummariesAndCounts(null, waterfallCallback);
		}
	], function(err){
...
db.createSummariesAndCounts = function(err, callback) {
    ...
    promise.addBack(function(err, hits){
        async.eachLimit(hits, 1, function(hit, eachCallback){
            ...
            console.log("Finished creating createSummariesAndCounts");
            callback(err);
        });
    });

    /*
    var stream = db.Music.find({}).select("artist sortArtist album").lean().stream();
	stream.on("data", function(music){
		async.waterfall([
			function(waterfallCallback) {
				db.updateArtists(music, waterfallCallback);
			},
			function(waterfallCallback){
				db.updateAlbums(music, waterfallCallback);
			}]);
	}).on("error", function(err){
		callback(err);
	}).on("close", function(){
		console.log("Finished music stream");
		//callback();
	});
    */
    /*
