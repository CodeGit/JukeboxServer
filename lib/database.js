...
db.updateArtists = function(data, callback) {
	...
	async.waterfall([
         function(callback){
        	 db.Artist.findOne({"artist":data.artist}, function(err, hit){
        		 if (err) {callback(err, hit);}
        		 callback(null, hit);
        	 });
         },
         function(dbArtist, callback){
        	 if (dbArtist == null) {
        		 console.log("Creating new " + data.artist);
        		 dbArtist = new db.Artist();
        		 dbArtist.artist = data.artist;
        		 dbArtist.sortArtist = data.sortArtist;
        		 dbArtist.music = 0;
        	 } else {
        		 db.Artist.music += 1;
        	 }
        	 console.log("Updating " + dbArtist.artist + " music = " + db.Artist.music);
        	 dbArtist.save(callback);
         }
	]);
};
