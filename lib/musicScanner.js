...
scanner._createTrack = function(file, tags) {
	...
	music.name = tags.title ? tags.title : "";
	music.artist = tags.artist ? tags.artist[0] : "";
	music.track = tags.track && tags.track.no ? tags.track.no : 0;
	music.total = tags.track && tags.track.of ? tags.track.of : 0;
	music.album = tags.album ? tags.album : "";
	music.year = tags.year ? tags.year : 0;
	music.genre = tags.genre ? tags.genre[0] : "";
	if (directoryImages.hasOwnProperty(path.dirname(file))) {
		music.image = directoryImages[path.dirname(file)][0];
	}
	db.Music.findOneAndUpdate({"path": file}, music, {upsert: true}, function (err, result){
		if (err) {
			console.log("MongoDB music error");
			scanner._handleError(err);
		}
		console.log("Wrote " + music.name + " to db");
	});
	/*
	length: Number,
	startSilence: Number,
