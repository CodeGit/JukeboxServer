...
m3u.readFileLoop = function() {
	console.log("inProgress = " + m3u.inProgress + " files = " + m3u.playlists.length);
	while (m3u.inProgress < m3u.readLimit && m3u.playlists.length > 0) {
		var data = m3u.playlists.shift();
		m3u.readPlaylist(data.dir, data.file);
		m3u.inProgress++;
	}
	//
};
...
m3u.processM3U = function(dir, playlistFile, lines) {
	...
	//move onto next playlist if called from readFileLoop()
	m3u.inProgress--;
	if (m3u.playlists.length > 0) {
		m3u.readFileLoop();
	}
};
