	...
	for(var i = 0; i < lines.length; i++) {
		...
		if (line.indexOf("#") !== 0) {
			...
			promise.addBack(function(err, hit){
				...
				m3u.addToPlaylist(path.basename(playlistFile), musicPath);
			});
		}
	}
};
...
m3u.addToPlaylist = function(playlistName, musicPath) {
	console.log("Adding "+ musicPath + " to playlist: " + playlistName);
	db.updatePlaylist(playlistName, musicPath);
};
